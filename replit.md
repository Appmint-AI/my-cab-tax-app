...
- **Industry Segment System**: Dual-industry support for Taxi/Rideshare and Delivery Courier segments. Users choose their segment on first login via `/pick-industry` (IndustryPickerPage). The `user_segment` field on the users table drives dynamic UI: segment-aware dashboard headings, earnings labels, expense suggestions, income source highlights, receipt scanning optimization, sidebar branding, and pro tips. A "Switch Industry" toggle in Settings allows hybrid drivers to swap views anytime. Config lives in `client/src/lib/segment-config.ts`.
