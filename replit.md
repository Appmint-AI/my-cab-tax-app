...
- **Finalize Submission Modal**: Full-screen modal with pre-submission validation results, three mandatory liability checkboxes (bookkeeping tool acknowledgment, income verification against 1099-K, vault authorization), and SHA-256 submission hash. All finalization actions are logged to `audit_logs` with IP address, user agent, submission hash, and all acknowledgment flags.
